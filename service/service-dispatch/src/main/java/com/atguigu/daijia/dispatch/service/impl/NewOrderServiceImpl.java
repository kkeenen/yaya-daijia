...
import static com.atguigu.daijia.common.constant.RedisConstant.DRIVER_ORDER_TEMP_LIST;
...
public class NewOrderServiceImpl implements NewOrderService {
    ...
    public Long addAndStartTask(NewOrderTaskVo newOrderTaskVo) {
        ...
        System.out.println("addAndStartTask:: orderjob :" + orderJob);
        //2 没有启动，进行操作
        if(orderJob == null) {
            //创建并启动任务调度
            ...
            System.out.println(newOrderTaskVo.getOrderId());
            orderJob.setJobId(jobId);
            orderJob.setParameter(JSONObject.toJSONString(newOrderTaskVo));
            orderJobMapper.insert(orderJob);
            System.out.println("orderJob :" + orderJob);
        }
    ...
    public List<NewOrderDataVo> findNewOrderQueueData(Long driverId) {
        ...
        System.out.println(key+" "+size + " " + list);
        return list;
//        List<NewOrderDataVo> list = new ArrayList<>();
//        String key = DRIVER_ORDER_TEMP_LIST + driverId;
//        Long size = redisTemplate.opsForList().size(key);
//        if(size > 0) {
//            for (int i = 0; i < size; i++) {
//                String content = (String)redisTemplate.opsForList().leftPop(key);
